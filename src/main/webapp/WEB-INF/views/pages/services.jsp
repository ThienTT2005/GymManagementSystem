            ...
            <c:choose>
                <c:when test="${not empty services}">
                    <c:forEach var="service" items="${services}">
                        <a href="${pageContext.request.contextPath}/services/${service.serviceId}" style="text-decoration: none; color: inherit; display: block;">
                            <article class="service-card">
                                <c:choose>
                                    <c:when test="${empty service.image}">
                                        <img src="${pageContext.request.contextPath}/images/bodycombat.png" alt="<c:out value='${service.serviceName}'/>" class="service-card-image">
                                    </c:when>
                                    <c:otherwise>
                                        <img src="${pageContext.request.contextPath}<c:out value='${service.image}'/>" alt="<c:out value='${service.serviceName}'/>" class="service-card-image">
                                    </c:otherwise>
                                </c:choose>
                                <div class="service-card-content">
                                    <h3 class="service-card-title"><c:out value="${service.serviceName}" /></h3>
                                    ...
                                    <p class="service-card-description">
                                        <c:choose>
                                            <c:when test="${fn:length(service.description) > 100}">
